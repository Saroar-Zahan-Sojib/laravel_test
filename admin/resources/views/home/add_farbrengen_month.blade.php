            ...
            <div class="col-md-6 pd0 float-left">
                <div class="card card-primary">
                    ...
                    @endif
                     ...
                     @endif
                    <div class="card-body">
                        <form method="post" action="{{ url('save-farbrengen-month') }}" enctype="multipart/form-data">
                        @csrf
                      ...
                      <tr>
                        ...
                        <tbody>
                            @foreach($data as $key => $dt)
                              <tr>
                                <td>{{$key+1}}</td>
                                <td class="name">{{$dt->month}}</td>
                                <td style="white-space: nowrap;">
                                    <a id="{{$dt->id}}" href="javascript:" class="btn btn-info btn-sm edit"> <i class="fa fa-edit"></i> </a> <a id="{{$dt->id}}" onclick="return confirm('Are you sure to delete this Category?')" href="{{ url('delete-category/'.$dt->id) }}" class="btn btn-danger btn-sm"> <i class="fa fa-trash"></i> </a>
                                </td>
                              </tr>
                            @endforeach    
