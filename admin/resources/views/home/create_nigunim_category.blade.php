            ...
            <div class="col-md-6 pd0 float-left">
                <div class="card card-primary">
                    ...
                    @endif
                     ...
                     @endif
                    <div class="card-body">
                        <form method="post" action="{{ url('save-nigunim-category') }}" enctype="multipart/form-data">
                        @csrf
                            <div class="form-group row nShow mb-1">
                                ...
                                <div class="col-md-8">
                                    <select class="form-control" name="nigunim_id" id="nigunim_id" readonly>
                                        ...
                                        @foreach($cat as $val)
                                        <option value="{{$val->id}}" {{ ( $val->name == 'Nigunim') ? 'selected' : '' }}>{{$val->name}}</option>
                                        @endforeach
                                    </select>
                            ...
                            @foreach($data as $key => $dt)
                              <tr>
                                ...
                                <td style="white-space: nowrap;">
                                    <a id="{{$dt->id}}" href="javascript:" class="btn btn-info btn-sm edit"> <i class="fa fa-edit"></i> </a> <a id="{{$dt->id}}" onclick="return confirm('Are you sure to delete this Category?')" href="{{ url('delete-nigunim-category/'.$dt->id) }}" class="btn btn-danger btn-sm"> <i class="fa fa-trash"></i> </a>
                                </td>
                              </tr>
                            @endforeach    
